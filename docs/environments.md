...
Staging has **its own Upstash database**. It must never share production's, because production
Redis holds encrypted patient health information.

Staging also gets its own `JWT_SECRET`, `PHI_ENCRYPTION_KEY`, and `ADMIN_KEY`. They are throwaway
values with no relationship to the production ones. A staging token must not authenticate against
production, and staging must not be able to decrypt production records.
...
2. **Create the staging Upstash database.** Vercel → `egovmed-backend` → Storage → Upstash for
   Redis → new free database. Scope it to **Preview only**. Do not reuse the production one.
3. **Set the remaining Preview env vars** (see below).
...
UPSTASH_REDIS_REST_URL=<staging database>
UPSTASH_REDIS_REST_TOKEN=<staging database>
```
